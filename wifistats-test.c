...
void startSort (int sortCol) {
    ...
    //Make sure that the correct parameters are supplied
    assert(sortCol == 2 || sortCol == 3);
    //Checks to see which field to sort by.
    //If there are 2 fields
    if (sortCol == 2) { //FIX HERE
...
int analyseMacAd(char mode, char packetsName[], int macAdList[MAX_MAC][ (MAC_AD_BYTES + 1) ] ) {
    printf("analyseMacAd\n");
    ...
    while (fgets(line, sizeof line, packets) != NULL) {
        ...
        if (!strcmp(rAd, "ff:ff:ff:ff:ff:ff") || !strcmp(rAd, "FF:FF:FF:FF:FF:FF") ) {
        ...
        //Checks to see if the recieving addresses are to be analysed
            ...
            sscanf(rAd, "%2x:%2x:%2x:%2x:%2x:%x", &checkAd[0], &checkAd[1], &checkAd[2], &checkAd[3], &checkAd[4], &checkAd[5]);
        }
    ...
    fclose(output); */
    //0 is returned to indicate successful running of the function
    return noAd;
}
...
int analyseOUI (char OUIFileName[], int macAdList[MAX_MAC][ (MAC_AD_BYTES + 1) ], int noAd) {
    printf("analyseOUI\n");
    ...
    int newMacAdList[MAX_MAC][ (OUI_BYTES + 1) ];
    //This is an int to hold the packet size data for the MAC address being looked at
    //int packetSiz = 0;
    //This is an int holding the number of MAC addresses in the macAdList array, this is different to the number of MAC addresses that have been processed as this looks at unique MAC addresses
    int newNoAd = 0;
    ...
    //while (fgets(line, sizeof line, output) != NULL) {
        ...
        //sscanf(ad, "%x:%x:%x:%*x:%*x:%*x", &checkAd[0], &checkAd[1], &checkAd[2]);
    ...
    for (int k = 0; k < noAd; k++) {
        if (newNoAd == 0) {
            ...
            for (int i = 0; i < OUI_BYTES; i++) {
                newMacAdList[newNoAd][i] = macAdList[noAd][i];
            }
            //This adds the packet information onto the end of the array as the final piece to be added
            newMacAdList[newNoAd][OUI_BYTES] = macAdList[noAd][MAC_AD_BYTES];
            //This adds one to the number of MAC addresses on the collated array as something has now been added
            newNoAd++;
        }
        //If its not on the first MAC address to be added it has to be compared to the MAC addresses already on the array incase it has the same address
            ...
            for (int i = 0; i <= newNoAd; i++) {
                ...
                else if (newMacAdList[i][0] == macAdList[noAd][0] && newMacAdList[i][1] == macAdList[noAd][1] && newMacAdList[i][2] == macAdList[noAd][2]) {
                    //If the address matches then the packet information is added to the existing paket information
                    newMacAdList[i][OUI_BYTES] += macAdList[noAd][MAC_AD_BYTES];
                    //And the loop is exited to start with the next address
                    break;
                }
            }
    ...
    fclose(output);
    //Exit with a zero to indicate success
    for (int i = 0; i < newNoAd; i++) {
        printf("%02x:%02x:%02x\t%s\t%i\n", newMacAdList[i][0], newMacAdList[i][1], newMacAdList[i][2], vendorName, newMacAdList[i][3]);
    }
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    char mode = *argv[1];
    int output[MAX_MAC][ (MAC_AD_BYTES + 1) ];
    int noAd;
    ...
    //Checks to see if the correct argument input has been supplied for analysing the packet file
        ...
        noAd = analyseMacAd(*argv[1], argv[2], output);
